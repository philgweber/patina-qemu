  ...
  )
{
  ...
  PATINA_PERFORMANCE_CONFIG_HOB  *PerfConfigHob;
  UINT64                         TpmBase;
  UINT32                         TpmSize;
  ...
  PerfConfigHob = BuildGuidHob (
                    &gPatinaPerformanceConfigHobGuid,
                    sizeof (*PerfConfigHob)
                    );
  if (PerfConfigHob != NULL) {
    PerfConfigHob->Enabled             = FALSE;
    PerfConfigHob->EnabledMeasurements = 0;

 #if defined (PERF_TRACE_ENABLE) && (PERF_TRACE_ENABLE)
    PerfMask = PcdGet8 (PcdPerformanceLibraryPropertyMask);
    if ((PerfMask & PERFORMANCE_LIBRARY_PROPERTY_MEASUREMENT_ENABLED) != 0) {
      PerfConfigHob->Enabled             = TRUE;
      PerfConfigHob->EnabledMeasurements = PerfMask;
    }
    ...
    DEBUG ((
      DEBUG_ERROR,
      "%a: Failed to allocate PATINA_PERFORMANCE_CONFIG_HOB\n",
      __func__
      ));
  }
